...
<plugin key="SSZone" name="AC Security zone" author="Erwanweb" version="0.0.2" externallink="https://github.com/Erwanweb/BoilerCLite.git">
    ...
    <params>
        ...
        <param field="Mode5" label="Detection delay, Alarm On delay, Alarm Off delay (all in seconds)" width="100px" required="true" default="10,30,10"/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if len(params) == 3:
            self.Detectiondelay = CheckParam("delay before detection validation)",params[0],10)
            self.Alarmondelay = CheckParam("Alarm On Delay",params[1],30)
            self.Alarmoffdelay = CheckParam("Alarm Off Delay",params[1],10)
    ...
    def AlarmDetection(self):
       ...
       # Build list of Alarm sensor (switches), with their current status
        ...
        if self.Alarmtempo + timedelta(seconds = self.Detectiondelay) >= now:
            ...
            Domoticz.Log("At mini 1 Alarm sensor still ON or was several time ON in detection delay...")
        else:
            self.NewDetection = False
